...
class IVDetect(nn.Module):
    ...
    def forward(self, g = None, dataset = None, graph = None , feat = None , eweight = None ,need_graph_feat_vec = False ,
                output_tSNE_embedding = False, tSNE_embedding_shape = -1 ):
        """Forward pass.
        ...
        if output_tSNE_embedding:
            ...
            for sub_g in dgl.unbatch(g):
                hidden_embedding = sub_g.ndata['h'].flatten().detach().cpu()
                # append zero to tSNE_embedding_shape shape
                hidden_embedding = torch.concat([ hidden_embedding , torch.zeros( tSNE_embedding_shape - hidden_embedding.shape[0] ) ])
                embedding.append( hidden_embedding )
        ...
        if need_graph_feat_vec:
            return batch_pooled, feat_vec.clone().detach()

        if output_tSNE_embedding:
            return batch_pooled ,embedding
