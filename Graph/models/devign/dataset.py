...
from utils.utils import processed_dir, dfmp, get_dir, cache_dir, tokenize , debug
...
type_map = {'UNKNOWN': 0, 'METHOD': 1, 'METHOD_PARAMETER_IN': 2, 'BLOCK': 3, 'External Function Call': 4,
            ...
            'JUMP_TARGET': 25, 'SWITCH': 26, 'BREAK': 27, 'DO': 28, 'CONTINUE': 29, 'TYPE_DECL': 30, 'MEMBER': 31 , 'TRY' : 32 ,  'METHOD_REF' : 33 , 'NAMESPACE_BLOCK' : 34}
...
class BigVulDatasetDevign(BigVulDataset):
    ...
    def __init__(self, need_code_info=False, **kwargs):
        """Init."""
        super(BigVulDatasetDevign, self).__init__(**kwargs)
        self.need_code_info = need_code_info
        ...
        ret = dfmp(
            self.df,
            partial(BigVulDatasetDevign._feat_ext_itempath,partition = self.partition),
            "id",
            ordr=True,
    ...
    def _feat_ext_itempath(_id,partition:str):
        ...
        code, _, _, _, _, _, counter = feature_extraction(BigVulDataset.itempath(_id,partition))
    ...
    def __getitem__(self, idx):
        """Override getitem."""
        _id = self.idx2id[idx]
        code, lineno, nt, ei, eo, et, _ = feature_extraction(BigVulDataset.itempath(_id,self.partition))
        g = dgl.graph((eo, ei))
        ...
        if self.need_code_info:
            node_code_info = []
            for c, l in zip(code, lineno):
                if l == "":
                    node_code_info.append((-1, c))
                else:
                    node_code_info.append((int(l) - 1, c))
            return g, node_code_info
        else:
            return g
...
def feature_extraction(filepath, graph_type="all", return_nodes=False):
    ...
    # print('feature_extraction cache name',cache_name)
