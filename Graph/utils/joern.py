...
from utils.utils import debug, external_dir, subprocess_cmd
...
def run_joern(filepath: str, verbose: int):
    ...
    params = f"filename={filepath}"
    command = f"/home/IdeaProjects/joern/joern-cli/target/universal/stage/joern --script {script_file} --params='{params}'"
    if verbose > 2:
        debug(command)
    subprocess_cmd(command, verbose=verbose)
    try:
        shutil.rmtree(f'/home/source/LLMvsGraph/workspace/{filename}')
...
def get_node_edges(filepath: str, verbose=0):
    ...
    with open(str(outfile) + ".nodes.json", "r") as f:
        ...
        try:
            nodes = nodes[
                ["id", "_label", "name", "code", "lineNumber" , "columnNumber", "controlStructureType"]
            ]
    ...
    # edges.drop(columns=["id_x","id_y"] , inplace=True)  # drop generated columns
