...
from utils.utils import get_dir, processed_dir, cache_dir, hashstr, project_dir, watch_subprocess_cmd, dfmp, tokenize_lines, \
    tokenize
import pandas as pd
...
def glove(
    ...
    BUILDDIR = project_dir() / 'GloVe/build'
...
def generate_glove(dataset="vul4c_dataset", sample=False, cache=True, n=None):
    ...
    cache_path = Path(dataset_path) / dataset / 'train.json'
    ...
    samples.func = dfmp(
        samples, tokenize_lines, "func", cs=200, desc="Get lines: "
    )
    lines = [i for j in samples.func.to_numpy() for i in j]  # get corpus

    # Save corpus
    savedir = get_dir(processed_dir() / dataset / f"glove_{sample}")
    if os.path.exists(savedir / "vectors.bin"):
        print("Already trained Glove.")
        return
