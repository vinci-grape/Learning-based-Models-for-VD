...
from utils.utils import tokenize, processed_dir, get_dir, debug, cache_dir, dfmp, tokenize_lines
...
def generate_w2v(dataset="vul4c_dataset", sample=False, cache=True, embedding_size=100,
    ...
    # Preprocessing
    samples.func = dfmp(
        samples, tokenize_lines, "func", cs=200, desc="Get lines: ", workers=32
    )
    lines = [' '.join(j).split() for j in samples.func.to_numpy()]  # get corpus
